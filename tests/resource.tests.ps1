...
$defaultRules = 65000, 65001, 65500

# iterate through subscriptions - TODO parallelize this
...
$rsPool = [runspacefactory]::CreateRunspacePool($min, $max)
$rsPool.open()
...
foreach ($subscription in $subscriptions) {
    ...
    $runspace.Runspace.InitialSessionState = $initialSessionState
    $null = $runspace.AddScript($scriptblock)
    $null = $runspace.AddArgument($allowedVmExtensions)
    $null = $runspace.AddArgument($subscription)
    $null = $runspace.AddArgument($azureDatacenterIpRanges)
    $null = $runspace.AddArgument($defaultRules)
